...
class SystemGoods extends BaseController
{
    public function getList()
    {
        ...
        $count = Db::name('system_goods')
            ->where($where)
            ->where($u_where)
            ->where('delete_time', null)
            ->count();

        $list = Db::name('system_goods')->alias('g')
            ->where($where)
            ->where($u_where)
            ->where('delete_time', null)
            ->page($page)
...
//                $servername = Env::get('server.servername', 'http://api.feishi.vip');
                ...
                foreach ($data['data'] as $k => $v) {
                    ...
                    $res = Db::name('system_goods')->where('delete_time', null)->where(['uid' => $user['id'], 'title' => $v[0]])->find();
                    if (!$res && $v[0]) {
                        $arr[$k]['title'] = $v[0];
                        ...
                        $res = Db::name('system_goods')->insert($arr[$k]);
                    }
                }
                unlink($filename);
                $data = [
