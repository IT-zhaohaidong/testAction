...
class Wxpay
{
    ...
    public function prepay($openid, $order_sn, $total_fee, $user, $notify_url)
    {
        ...
        if ($is_set_mchid && $wx_mchid_id) {
            $mchid = (new MchidModel())->where('id', $wx_mchid_id)->find();
            $this->config['mchid'] = $mchid['mchid'];
            $this->config['key'] = $mchid['key'];
        }
        $unifiedorder = array(
...
//        if (($sign === $data_sign) && ($data['return_code'] == 'SUCCESS')) {
        ...
        (new Goods)->orderDeal($result, 1, 1);
...
//        if (($sign === $data_sign) && ($data['return_code'] == 'SUCCESS')) {
        ...
        (new FinanceOrder())->where('order_sn', $out_trade_no)->update(['openid' => $openid]);
        (new Goods)->orderDeal($result, 3, 1);
